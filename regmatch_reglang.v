...
Canonical Structure re_eqType := EqType _ r_eqMixin.

Fixpoint regexp2r (r0 : regexp char) : r char :=
  match r0 with
  | Void => r_zero
  | Eps => r_unit
  | Atom c' => r_char c'
...
Lemma cancel_re_regexp : cancel r2regexp regexp2r.
Proof.
rewrite /cancel.
elim => //=.
- move => r IH r' IH'.
...
Lemma regexp_re_in : forall (r : r char) (w : seq char), s_in_regexp_lang _ w r -> w \in re_lang (r2regexp r).
Proof.
move => r w H.
remember r as r0.
remember w as w0.
...
Lemma regexp_re_in' : forall (r : regexp char) (w : seq char), s_in_regexp_lang _ w (regexp2r r) -> w \in re_lang r.
Proof.
move => r w H.
...
refine
  (fun r w => 
     match accept (@eq_comparable char) (regexp2r r, w) with
     | left H_l => left _
     | right H_r => right _
     end).
- exact: regexp_re_in'.
