...
Instance wf_accept_lt : WellFounded accept_lt.
apply accept_lt_well_founded.
Defined.

Equations acc (rs : re char * list char) : accept_t rs by wf rs accept_lt :=
  acc (re_zero, _) := right _;
  acc (re_unit, []) := left _; acc (re_unit, _ :: _) := right _;
  acc (re_char c, [c']) := match char_eq_dec c c' with left _ => left _ | right _ => right _ end;
  acc (re_char _, []) := right _; acc (re_char _, _ :: _) := right _;
  acc (re_plus r1 r2, s) := match acc (r1, s) with left _=> left _ | right _ => match acc (r2, s) with left _ => left _ | right _ => right _ end end;
  acc (re_times r1 r2, []) := match acc (r1, []) with left _ => (match acc (r2, []) with left _ => left _ | right _ => right _ end) | right _ => right _ end;
  acc (re_times re_zero _, _) := right _;
  acc (re_times re_unit r2, c :: s) := match acc (r2, c :: s) with left _ => left _ | right _ => right _ end;
  acc (re_times (re_char c) r2, c' :: s) := match char_eq_dec c c' with left _ => (match acc (r2, s) with left _ => left _ | right _ => right _ end) | right _ => right _ end;
  acc (re_times (re_times r'1 r'2) r2, c :: s) := match acc (re_times r'1 (re_times r'2 r2), c :: s) with left _ => left _ | right _ => right _ end;
  acc (re_times (re_plus r'1 r'2) r2, c :: s) :=
    match acc (re_times r'1 r2, c :: s) with left _ => left _  | right _ => match acc (re_times r'2 r2, c :: s) with left _ => left _ | right _ => right _ end end;
  acc (re_times (re_star r1) r2, c :: s) :=
      match acc (r2, c :: s) with
      | left _ => left _ 
      | right _ => (match regexps_no_c (r1, c) with
                   | exist l H_l =>
                     match @accept_list_dec (re_times (re_star r1) r2, c :: s) acc (fun r0 => (re_times r0 (re_times (re_star r1) r2), s)) _ l with
                     | inleft (exist _ H_ex) => left _ 
                     | inright H_l' => right _
                     end
                   end)
      end;
  acc (re_star r, []) := left _;
  acc (re_star r, c :: s) :=
    (match regexps_no_c (r, c) with
     | exist l H_l => 
       match @accept_list_dec (re_star r, c :: s) acc (fun r0 => (re_times r0 (re_star r), s)) _ l with
       | inleft (exist _ H_ex) => left _
       | inright H_l' => right _ 
       end
     end).
...
(*
Equations regexps_no_c_F' (r : re char) (c : char) : regexps_no_c_t (r, c) :=
  regexps_no_c_F' re_zero _ := exist _ [] _;
  regexps_no_c_F' re_unit _ := exist _ [] _;
  regexps_no_c_F' (re_char c') c := match char_eq_dec c c' with
                                   | left _ => exist _ [re_unit] _
                                   | right _ => exist _ [] _ end;
*)
